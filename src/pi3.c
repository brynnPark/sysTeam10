...
int main(int argc, char **argv)
{
  ...
  //thread_1 온습도
  thr_id = pthread_create(&p_thread[0], NULL, temperature_control, (void *)&p3); 
  if (thr_id < 0)
  {
  ...
  // thread_1 물수위센서
  thr_id = pthread_create(&p_thread[3], NULL, water_control, (void *)&p1); // 온습
  if (thr_id < 0)
  {
    perror("thread create error : ");
    exit(0);
...
void *temperature_control(void *arg)
{
  ...
  while (1)
  {
    ...
    int_to_str(dht11_val[0], send, 10);
    int_to_str(dht11_val[2],send2,10);
    strcpy(a, "$");
    strcat(a, send);
    strcpy(b, "$");
    strcat(b, send2);
